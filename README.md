...
- ***Tipo de instrumentación → Manual***. (Es decir se utilizó la libreria micrometer y micrometer-traicing + sus bridges correspondientes de OpenTelemetry, para generar información de metricas y tracing respectivamente. Y para los logs se usó una libreria propia de OpenTelemetrySDK(Log SDK) que genera el bridge entre logback y otel. https://opentelemetry.io/docs/instrumentation/java/manual/      
- ***Tipo deployment del collector → As a Service*** (Es decir existe un container docker corriendo el servicio de otel-collector) https://opentelemetry.io/docs/collector/deployment/agent/
